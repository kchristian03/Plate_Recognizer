...
def get_db_connection():
    return mysql.connector.connect(
        host='153.92.9.14',
        user='u1652679_netdxc-admin',
        password='Admin-0321',
        database='u1652679_plate-recognizer'
    )
...
def predict():
    ...
    if imagefile and allowed_file(imagefile.filename):
        ...
        insert_query = """
        INSERT INTO predictions (image_name, timestamp, prediction)
        VALUES (%s, %s, %s)
        """
        cursor.execute(insert_query, (filename, timestamp, line))
        db_conn.commit()
        cursor.close()
        db_conn.close()
